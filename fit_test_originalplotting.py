...
for mcmcfile in os.listdir(directory):
    if mcmcfile.startswith('z3_fixedzred'): 
        ...
        print("output original:" + str(logmass))
        print("output integral mass: " + str(np.log10(trap(age_interp*1e9, sfrPercent[:,2]))))
        outputIntegralMass = np.log10(trap(age_interp*1e9, sfrPercent[:,2]))

        #LOGMASS
        ax[0,0].errorbar(obs['logM'],outputIntegralMass,marker='.', markersize=10, ls='', lw=2, 
        ...
        #SFRs
        '''
        ax[0,1].errorbar(um_sfh[-1], sfrPercent[:,2], marker='.', markersize=10, ls='', lw=2, 
       ...
       '''
        #LOGZSOL
        ax[1,0].errorbar(spsdict['logzsol'],logzsol,marker='.', markersize=10, ls='', lw=2, 
...
# save plot 
counter=0
filename = 'bigplot_originalplotting_z3_fixedzred_{}.pdf' #defines filename for all objects
