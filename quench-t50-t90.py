...
plotdir = '/Users/michpark/JWST_Programs/mockgalaxies/scatterplots-mb-nomb/quenchtest/'
...
for directory_index, directory in enumerate(directory_array):
    ...
    for mcmcfile in os.listdir(directory):
            ...
            print("For " + str(obs['objid']) + ", we have input t50: " + str(x_in_t50[0]) + ", rec t50: " + str(x_rec_t50[0]) + ", input t90: " + str(x_in_t90[0]) + ", rec t90: " + str(x_rec_t90[0]))
            
            # t50 in, t50 rec, t90 in, t90 rec, my quench in, my quench out
            
            if(directory_index == 0): # MB
                if(first_iteration):
                    results_mb = np.array([x_in_t50[0], x_rec_t50[0], x_in_t90[0], x_rec_t90[0], x_i[0], x_o[0]]).flatten()
                else:
                    results_mb = np.vstack([results_mb, np.array([x_in_t50[0], x_rec_t50[0], x_in_t90[0], x_rec_t90[0], x_i[0], x_o[0]]).flatten()])  
            if(directory_index == 1): # No MB
                if(first_iteration):
                    results_nomb = np.array([x_in_t50[0], x_rec_t50[0], x_in_t90[0], x_rec_t90[0], x_i[0], x_o[0]]).flatten()
                else:
                    results_nomb = np.vstack([results_nomb, np.array([x_in_t50[0], x_rec_t50[0], x_in_t90[0], x_rec_t90[0], x_i[0], x_o[0]]).flatten()])  
            first_iteration= False
            
# input vs. recovered t50
# input vs. recovered t90
# input vs. recovered t90-t50
# input my quench vs t90
# recovered my quench vs. t90
fig, ax = plt.subplots(2,3,figsize=(12,9))

# t50 in, t50 rec, t90 in, t90 rec, my quench in, my quench out
# medium bands
ax[0,0].plot(results_mb[:,0], results_mb[:,1], color='maroon', marker='.', markersize=10, ls='', alpha=0.7)
ax[0,1].plot(results_mb[:,2], results_mb[:,3], color='maroon', marker='.', markersize=10, ls='', alpha=0.7)
ax[0,2].plot(results_mb[:,2]-results_mb[:,0], results_mb[:,3]-results_mb[:,1], color='maroon', marker='.', markersize=10, ls='', alpha=0.7)
ax[1,0].plot(results_mb[:,2], results_mb[:,4], color='maroon', marker='.', markersize=10, ls='', alpha=0.7)
ax[1,1].plot(results_mb[:,3], results_mb[:,5], color='maroon', marker='.', markersize=10, ls='', alpha=0.7)

# no medium bands
ax[0,0].plot(results_nomb[:,0], results_nomb[:,1], color='navy', marker='.', markersize=10, ls='', alpha=0.7)
ax[0,1].plot(results_nomb[:,2], results_nomb[:,3], color='navy', marker='.', markersize=10, ls='', alpha=0.7)
ax[0,2].plot(results_nomb[:,2]-results_nomb[:,0], results_mb[:,3]-results_mb[:,1], color='navy', marker='.', markersize=10, ls='', alpha=0.7)
ax[1,0].plot(results_nomb[:,2], results_nomb[:,4], color='navy', marker='.', markersize=10, ls='', alpha=0.7)
ax[1,1].plot(results_nomb[:,3], results_nomb[:,5], color='navy', marker='.', markersize=10, ls='', alpha=0.7)
...
ax[0,1].set_ylabel(r'Recovered $t90$ [Gyr]')
ax[0,1].set_xlabel(r'Input $t90$ [Gyr]')

# input vs. recovered t90-t50
ax[0,2].axline((0, 0), slope=1., ls='--', color='black', lw=2)
ax[0,2].set_ylabel(r'Recovered $t90 - t50$ [Gyr]')
ax[0,2].set_xlabel(r'Input $t90 - t50$ [Gyr]')

# input my quench vs t90
ax[1,0].axline((0, 0), slope=1., ls='--', color='black', lw=2)
ax[1,0].set_ylabel(r'Input quench time [Gyr]')
ax[1,0].set_xlabel(r'Input $t90$ [Gyr]')

arrow_location = [3.8, 2.2, 1.2] 
label_location = [2.6, 1.5, 0.85] 
ax[1,0].axline((0, 0), slope=0, ls='-', color='black', lw=1.5)
#ax[1,0].arrow(arrow_location[round(spsdict['zred'])-1], 0, 0, -0.5, width=0.02, color=None, edgecolor='black') 
#ax[1,0].text(label_location[round(spsdict['zred'])-1], 0.1, "No quenching")
ax[1,0].set_ylim(bottom=-0.8)

# recovered my quench vs. t90
ax[1,1].axline((0, 0), slope=1., ls='--', color='black', lw=2)
ax[1,1].set_ylabel(r'Recovered quench time [Gyr]')
ax[1,1].set_xlabel(r'Recovered $t90$ [Gyr]')

ax[1,1].axline((0, 0), slope=0, ls='-', color='black', lw=1.5)
#ax[1,1].arrow(arrow_location[round(spsdict['zred'])-1], 0, 0, -0.5, width=0.02, color=None, edgecolor='black') 
#ax[1,1].text(label_location[round(spsdict['zred'])-1], 0.1, "No quenching")
ax[1,1].set_ylim(bottom=-0.8)
...
counter=0
filename = '{}_z3_quenchtest.pdf' #defines filename for all objects
