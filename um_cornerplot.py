...
def demo(ndim=3, nsample=int(1e4)):
    ...
    axes = cornerplot.allcorner(samples, labels, axes, show_titles=True,
                                psamples=means[:, None])
    pl.show()
    return axes
...
def allcorner(samples, labels, axes, weights=None, span=None,
    ...
    if (qcolor is not None) | show_titles:
        show_extras(samples, labels, axes, weights=weights,
                    qcolor=qcolor, truths=truths, show_titles=show_titles, title_kwargs=title_kwargs)
...
def show_extras(samples, labels, paxes, weights=None,
    ...
    for i, xx in enumerate(samples):
        ...
        if truths is not None and truths[i] is not None:
            try:
                [ax.axvline(t, color="red", **truth_kwargs)
                 for t in truths[i]]
            except:
                ax.axvline(truths[i], color="red", **truth_kwargs)
            ...
            if truths is not None:
                if truths[j] is not None:
                    try:
                        ...
                        ax.axvline(truths[j], color="black", **truth_kwargs)
                if truths[i] is not None:
                    try:
                        [ax.axhline(t, **truth_kwargs)
                         for t in truths[i]]
                    except:
                        ax.axhline(truths[i], color="black", **truth_kwargs)
        '''
